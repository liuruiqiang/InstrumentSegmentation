...
from network._deeplab import DeepLabHead, DeepLabHeadV3Plus, DeepLabV3,DeepLabHeadV3Plus_WthTrans
...
def _segm_resnet(name, backbone_name, num_classes, output_stride, pretrained_backbone):
    ...
    if name=='deeplabv3plus':
        ...
        classifier = DeepLabHeadV3Plus(inplanes, low_level_planes, num_classes, aspp_dilate)
    elif name == 'DeepLabHeadV3Plus_WthTrans':
        return_layers = {'layer4': 'out', 'layer1': 'low_level'}
        classifier = DeepLabHeadV3Plus_WthTrans(inplanes, low_level_planes, num_classes, aspp_dilate)
    elif name=='deeplabv3':
...
def _segm_transresnet(name, backbone_name, num_classes, output_stride, pretrained_backbone,
                      numheads=8,h=320,w=256,time_length=2):
    if output_stride == 8:
        replace_stride_with_dilation = [False, True, True]
        aspp_dilate = [12, 24, 36]
    else:
    ...
    if name == 'deeplabv3plus':
        ...
        return_layers = {'layer4': 'out', 'layer1': 'low_level'}
        classifier = DeepLabHeadV3Plus_WthTrans(inplanes, low_level_planes, num_classes, aspp_dilate
                                                ,numheads=numheads,h=h,w=w,time_length=time_length)
    elif name == 'deeplabv3':
        return_layers = {'layer4': 'out'}
        classifier = DeepLabHead(inplanes, num_classes, aspp_dilate)
    backbone = IntermediateLayerGetter(backbone, return_layers=return_layers)
...
def _load_model(arch_type, backbone, num_classes, output_stride, pretrained_backbone,
    ...
    if arch_type == 'DeepLabHeadV3Plus_WthTrans':
        model = _segm_transresnet(arch_type, backbone, num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone
                                  ,numheads=numheads,h=h,w=w,time_length=time_length)
        return model
...
def deeplabv3pluswthtrans_resnet50(num_classes=21, output_stride=8, pretrained_backbone=True,
                                   numheads=8,h=320,w=256,time_length=2):
    """Constructs a DeepLabV3 model with a ResNet-50 backbone.
    ...
    Args:
        ...
        pretrained_backbone (bool): If True, use the pretrained backbone.
    """
    return _load_model('DeepLabHeadV3Plus_WthTrans', 'resnet50', num_classes, output_stride=output_stride, pretrained_backbone=pretrained_backbone
                       ,numheads=numheads,h=h,w=w,time_length=time_length)


data = torch.rand(2,5,3,320,256).cuda()
model = deeplabv3plus_resnet50(8).cuda()
out = model(data)
# print(out)
