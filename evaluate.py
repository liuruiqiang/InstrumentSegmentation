...
if __name__ == '__main__':
    ...
    arg('--train_path', type=str, default='data/aug500e',
        help='path where train images with ground truth are located')
    arg('--target_path', type=str, default='1', help='path with predictions')
    ...
    if args.problem_type == 'binary':
        ...
        for instrument_id in tqdm(range(9, 19)):
            instrument_dataset_name = 'instrument_dataset_' + str(instrument_id)
            for file_name in (
                    Path(args.train_path) / instrument_dataset_name / 'parts_masks').glob('*'):
                y_true = cv2.imread(str(file_name), 0)
                y_true = cv2.copyMakeBorder(y_true, 0, 256, 0, 0, cv2.BORDER_CONSTANT, value=(0, 0, 0))
                y_true=image=cv2.resize(y_true,(960,960),interpolation=cv2.INTER_NEAREST)
                pred_file_name = Path(args.target_path) /  instrument_dataset_name / file_name.name
