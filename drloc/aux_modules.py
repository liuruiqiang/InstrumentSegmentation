...
def timedim_randn_sampling(maxinth,maxintw,time_length, sample_size, batch_size,):
    time_rand = torch.randint(time_length,size=(batch_size, sample_size,1))
    h_rand = torch.randint(maxinth,size=(batch_size, sample_size, 1))
    w_rand = torch.randint(maxintw, size=(batch_size, sample_size, 1))
    index_result = torch.cat([time_rand,h_rand,w_rand],dim=2)
    return index_result

def collect_time_samples_faster(feats, pxy, batch_size):
    n,c,t,h,w = feats.size()
    # print('size',n,c,t,h,w)
    feats = feats.view(n, c, -1).permute(1,0,2).reshape(c, -1)  # [n, c, t,h, w] -> [n, c, thw] -> [c, nthw]
    ranges = (torch.arange(n).long().to(pxy.device) * h * w).view(n, 1)
    # print((torch.arange(n).long().to(pxy.device) * t* h * w).view(n, 1) + pxy[:,:,0]*h*w)
    #pxy[:,:,0]*h+pxy[:,:,1]compute the position after the 2d map is flattened
    pxy = ((torch.arange(n).long().to(pxy.device) * t* h * w).view(n, 1) + pxy[:,:,0]*h*w + pxy[:,:,1]*w + pxy[:,:,2]).view(-1)  # [n, m, 2] -> [nm]
    return (feats[:,pxy]).view(c, n, -1).permute(1,0,2)
...
class DenseRelativeLoc(nn.Module):
    ...
    def forward_features(self, x, mode="part"):
        ...
        if mode == "part":
            pxs = randn_sampling(min(H,W), self.sample_size, B).detach()
            pys = randn_sampling(min(H,W), self.sample_size, B).detach()
...
class DenseRelativeLocTimedim(nn.Module):
    def __init__(self, in_dim, out_dim=3, sample_size=32,time_length=5, drloc_mode="l1", use_abs=False):
        super(DenseRelativeLocTimedim, self).__init__()
        self.sample_size = sample_size
        self.in_dim = in_dim
        self.drloc_mode = drloc_mode
    ...
    def forward_features(self, x, mode="part"):
        ...
        self.H = H
        self.W = W
        x = x.reshape(int(B/self.time_length),self.time_length,C,H,W).permute(0,2,1,3,4).contiguous()

        if mode == "part":
            pxs = timedim_randn_sampling(H,W,self.time_length, self.sample_size, int(B/self.time_length)).detach()
            pys = timedim_randn_sampling(H,W,self.time_length, self.sample_size, int(B/self.time_length)).detach()

            deltaxy = (pxs - pys).float().to(x.device)  # [B, sample_size, 3]

            ptsx = collect_time_samples_faster(x, pxs, B).transpose(1, 2).contiguous()  # [B, sample_size, C]
            ptsy = collect_time_samples_faster(x, pys, B).transpose(1, 2).contiguous()  # [B, sample_size, C]
        else:
            pts_norm, pts_rnd = collect_positions(B, H)
            ptsx = x.view(B, C, -1).transpose(1, 2).contiguous()  # [B, H*W, C]
            ptsy = collect_samples(x, pts_rnd, B).transpose(1, 2).contiguous()  # [B, H*W, C]
    ...
    def forward(self, x, normalize=False):
        pred_feats, deltaxy, H = self.forward_features(x)
        deltaxy = deltaxy.view(-1, 3)  # [B*sample_size, 3]

        if self.use_abs:
            deltaxy = torch.abs(deltaxy)
            if normalize:
                deltaxy /= torch.tensor([float(self.time_length - 1),float(self.H - 1),float(self.W - 1)])
        else:
            deltaxy += (H - 1)
            if normalize:
                deltaxy /= float(2 * (H - 1))
