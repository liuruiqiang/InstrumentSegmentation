...
class Attention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.0, num_patches_space=None, num_patches_time=None, attn_type=None):
        ...
        assert attn_type in ['space', 'time','fusion'], 'Attention type should be one of the following: space, time.'
    ...
    def forward(self, x):
        ...
        elif self.attn_type == 'fusion':
            out = self.forward_fusion(x) # (b, tn, d)
...
class Transformer(nn.Module):
    def __init__(self, dim, heads, dim_head, mlp_dim, dropout, num_patches_space, num_patches_time):
        ...
        # self.attention_fusion = PreNorm(dim,Attention(dim, heads=heads_half, dim_head=dim_head, dropout=dropout, num_patches_space=num_patches_space, num_patches_time=num_patches_time, attn_type='space'))
...
class Transformer_fusion(nn.Module):
    def __init__(self, dim, heads, dim_head, mlp_dim, dropout, num_patches_space, num_patches_time):
        ...
        self.num_patches_space = num_patches_space
        self.num_patches_time = num_patches_time
        heads_third = int(heads / 3.0)

        assert dim % 2 == 0

        self.attention_space = PreNorm(dim, Attention(dim, heads=heads_third, dim_head=dim_head, dropout=dropout, num_patches_space=num_patches_space, num_patches_time=num_patches_time, attn_type='space'))
        self.attention_fusion = PreNorm(dim,Attention(dim, heads=heads_third, dim_head=dim_head, dropout=dropout, num_patches_space=num_patches_space, num_patches_time=num_patches_time, attn_type='fusion'))
        self.attention_time = PreNorm(dim, Attention(dim, heads=heads_third, dim_head=dim_head, dropout=dropout, num_patches_space=num_patches_space, num_patches_time=num_patches_time, attn_type='time'))

        inner_dim = dim_head * heads_third * 3
        self.linear = nn.Sequential(nn.Linear(inner_dim, dim), nn.Dropout(dropout))
        self.mlp = PreNorm(dim, FeedForward(dim, mlp_dim, dim, dropout=dropout))
    ...
    def forward(self, x):
        ...
        xs = self.attention_space(x)
        xt = self.attention_time(x)
        x_fusion = self.attention_fusion(x)
        out_att = torch.cat([xs,x_fusion, xt], dim=2)
...
class Transformer_fusionv2(nn.Module):
    def __init__(self, dim, heads, dim_head, mlp_dim, dropout, num_patches_space, num_patches_time):
        super().__init__()
        ...
        # self.attention_space = PreNorm(dim, Attention(dim, heads=heads_half, dim_head=dim_head, dropout=dropout, num_patches_space=num_patches_space, num_patches_time=num_patches_time, attn_type='space'))
        self.attention_fusion = PreNorm(dim,Attention(dim, heads=heads_half, dim_head=dim_head, dropout=dropout, num_patches_space=num_patches_space, num_patches_time=num_patches_time, attn_type='fusion'))
        self.attention_time = PreNorm(dim, Attention(dim, heads=heads_half, dim_head=dim_head, dropout=dropout, num_patches_space=num_patches_space, num_patches_time=num_patches_time, attn_type='time'))
...
data = torch.rand(2,3*10*8,512).cuda()
model = Transformer_fusionv2(dim=512,heads=24,dim_head=512,mlp_dim=512*4,dropout=0.1,
                    num_patches_space=80,num_patches_time=3).cuda()
out = model(data)
print(out.shape)
